...
def test_preprocess_function():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert x_train_loaded.shape[0] == 455, f"x_train should have 455 samples, but got {x_train_loaded.shape[0]}"
        assert x_test_loaded.shape[0] == 114, f"x_test should have 114 samples, but got {x_test_loaded.shape[0]}"
        assert y_train_loaded.shape[0] == 455, f"y_train should have 455 samples, but got {y_train_loaded.shape[0]}"
        assert y_test_loaded.shape[0] == 114, f"y_test should have 114 samples, but got {y_test_loaded.shape[0]}"
